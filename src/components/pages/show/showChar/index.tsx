...
function ShowChar() {
  ...
  return (
   <section>
        <video src={datachars[char??'ratu']} autoPlay loop className='showCharVideo'></video>
        {
            filteredDataChar.map((data )=>( 
                    <div className={`show__char__base__card ${styleCharsPacks[char||'ratu'].card}`}key={id} 
                        onMouseEnter={()=>{
                    ...
                    >
                        ...
                        <div className="showChar_Link__wrapper translate-5" id='showlink'>
                            <NavLink to={'/'} className={`showChar_Link  ${styleCharsPacks[char??'ratu'].link}`}>
                            back to home
                        </NavLink>
                        </div>
                    </div>
